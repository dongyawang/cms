...
<table>
            <tr><td colspan="5" style="background-color:#F1EFEF;text-align:left;">招聘文章 >> 分类列表</tr>
            <tr><td>选中</td><td>分类排序</td><td>分类名称</td><td>修改分类</td><td>更新时间</td></tr>
<?php
$pagesize=20;  //定义每页显示记录条数
$page=isset($_GET["page"])?intval($_GET["page"]):1;   //定义page的初始值,如果get 传过来的page为空,则page=1,
$total=mysql_num_rows(mysql_query("select * from cms_recruitment_type order by id desc"));  //执行查询获取总记录数
...
$exec="select * from cms_recruitment_type order by id desc limit $offset,$pagesize"; //执行查询
