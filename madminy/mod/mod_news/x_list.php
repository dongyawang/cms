...
$total=mysql_num_rows(mysql_query("select * from cms_news_list order by id desc"));  //执行查询获取总记录数
...
$exec="select * from cms_news_list order by id desc limit $offset,$pagesize"; //执行查询
...
while ($row=mysql_fetch_array($result)){  //循环读取记录
   ...
   echo "<td>${id}</td>"; 
   echo "<td>${type}</td>";  
   echo "<td><a href=\"../../../news_details_${id}.html\" target=\"_blank\">${title}</a></td>";  
   echo "<td><a href=\"news_modify_${id}.html\"><img src=\"../../Images/modify.gif\" border=\"0\" width=\"58\" height=\"21\" /></a></td>"; 
