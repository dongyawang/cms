...
<body>
<form id="form1" name="form1" method="post" action="link_del.php">
<table>
            <tr><td colspan="7" style="background-color:#F1EFEF;text-align:left;">友情链接 >> 链接列表</tr>
            <tr><td>选中</td><td>链接排序</td><td>网站名称</td><td>网站地址</td><td>备注内容</td><td>修改链接</td><td>更新时间</td></tr>
<?php
$pagesize=20;  //定义每页显示记录条数
$page=isset($_GET["page"])?intval($_GET["page"]):1;   //定义page的初始值,如果get 传过来的page为空,则page=1,
$total=mysql_num_rows(mysql_query("select id from cms_links"));  //执行查询获取总记录数
...
$exec="select * from cms_links order by rank asc limit $offset,$pagesize"; //执行查询
...
while ($row=mysql_fetch_array($result)){  //循环读取记录
   ...
   echo "<td>${url}</td>";  
   echo "<td>${note}</td>";  
   echo "<td><a href=\"link_modify.php?a=${id}\"><img src=\"../../Images/modify.gif\" border=\"0\" width=\"58\" height=\"21\" /></a></td>";  
